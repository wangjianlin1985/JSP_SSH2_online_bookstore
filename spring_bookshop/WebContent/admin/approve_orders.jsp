        ...
        <s:iterator var="order" value="#request.orders">
            <tr>
               ...
               <td>￥<s:property value="#order.totalPrice"/></td>
                ...
                <c:if test="${order.orderStatus==0 }">
	                <td><a href="javascript:void(0);" onclick="javascript:if(confirm('确定锁定该定单吗？')){location='/spring_bookshop/lockOrder.do?item=${order.oid }';};">
	                <span class="editSpan"></span></a></td>
                </c:if>
	            <c:if test="${order.orderStatus==1 }">
	                <td><a href="javascript:void(0);" onclick="javascript:if(confirm('确定解锁该定单吗？')){location='/spring_bookshop/unlockOrder.do?item=${order.oid }';};">
	                <span class="editSpan"></span></a></td>
                </c:if>
            </tr>
         </s:iterator>   
